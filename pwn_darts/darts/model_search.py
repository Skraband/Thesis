...
class MixedOp(nn.Module):
    ...
    def forward(self, batch_x, batch_y, weights,srnn_arch_weights, spectral_transfomer, spn):
        out_1 = 0
        out_2 = 0
...
class Cell(nn.Module):
    ...
    def __init__(self, steps, spectral_transfromer, srnn, transformer, wein, cwspn):
        ...
        for i in range(self._steps):
            op = MixedOp(spectral_transfromer, srnn, transformer, wein, cwspn)
            self._ops.append(op)
    ...
    def forward(self, batch_x, batch_y, weights,srnn_arch_weights, spectral_transfomer, spn):
        ...
        if spectral_transfomer:
            out_1, out_2 = self._ops[0](batch_x, batch_y, weights,srnn_arch_weights, spectral_transfomer, spn)
        else:
...
class Network(nn.Module):
    ...
    def __init__(self, srnn, transformer, wein, cwspn, cwspn_spn, layers, criterion,search_srnn=False,search_cwspn=False,smape_target=False, steps=1, multiplier=4,
                 stem_multiplier=3):
        ...
        for i in range(layers):
            ...
            cell = Cell(steps, spectral_transfromer, srnn, transformer, wein, cwspn)
    ...
    def forward(self, batch_x, batch_y, batch_westimator_x_val, batch_westimator_y_val):
        ...
        #  s0, s1 = s1, cell(s0, s1,batch_y, weights)
        ...
        for i, cell in enumerate(self.cells):
            ...
            if cell.spectral_transfromer:
                weights = F.softmax(self.alphas_normal, dim=-1)
                prediction, f_c = cell(batch_x, batch_y, weights,srnn_arch_weights, spectral_transfomer=True, spn=spn,)
                my_f_c = f_c.reshape((f_c.shape[0], -1)).detach()
            else:
                weights = F.softmax(self.alphas_reduce, dim=-1)
                prediction_ll, _ = cell(batch_westimator_x_val, my_f_c, weights,srnn_arch_weights, spectral_transfomer=False, spn=spn)
